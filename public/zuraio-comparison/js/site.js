import { nav, footerGroups } from './copy-en.js';
...
function pathPrefix() {
  const depth = (location.pathname.match(/zuraio-comparison/g) || []).length;
...
function href(path) {
  ...
  const base = location.pathname.includes('zuraio-comparison') ? '' : '';
  return `${base}${path}`;
}
...
export function renderHeader() {
  ...
  const techLinks = nav.technical
    .map((l) => `<a href="${href(l.href)}" role="menuitem">${l.label}</a>`)
    .join('');

  el.innerHTML = `
    <header class="nav" id="nav">
      <div class="wrap nav-in">
        <a class="brand" href="${href('index.html')}" aria-label="Zuraio home">
          ${LOGO_SVG}
        ...
        <div class="nav-menu" id="nav-menu">
          <nav class="nav-links" aria-label="Primary">
            ${nav.main.map((l) => `<a href="${href(l.href)}">${l.label}</a>`).join('')}
            <div class="nav-dropdown">
              <button type="button" class="nav-dropdown-btn" aria-expanded="false" aria-haspopup="true">Technical</button>
              <div class="nav-dropdown-menu" role="menu">${techLinks}</div>
            </div>
            <a href="${href(nav.about.href)}">${nav.about.label}</a>
          </nav>
          <div class="nav-actions">
            <div class="lang-switch" aria-label="Language">
              <button type="button" class="lang-btn is-active" aria-pressed="true">EN</button>
              <button type="button" class="lang-btn" aria-pressed="false" title="German coming soon">DE</button>
            </div>
            <a class="btn btn-primary" href="${href('contact.html')}">Book a demo</a>
          </div>
        </div>
      </div>
    </header>`;
  ...
  el.querySelectorAll('.lang-btn').forEach((btn) => {
    btn.addEventListener('click', () => {
      if (btn.textContent === 'DE') {
        alert('German translation coming soon.');
      }
    });
  });
  ...
  const groups = footerGroups
    .map(
      ...
      <div class="foot-col">
        ...
        <ul class="foot-col-links">${g.links.map((l) => `<li><a href="${href(l.href)}">${l.label}</a></li>`).join('')}</ul>
      </div>`,
    )
    .join('');
    ...
    <footer class="site-footer">
      <div class="wrap">
        <div class="foot-grid">
          ${groups}
          <div class="foot-col">
            <h3 class="foot-col-title">Language / Contact</h3>
            <ul class="foot-col-links">
              <li><span class="foot-muted">EN (active)</span></li>
              <li><span class="foot-muted">DE (coming soon)</span></li>
